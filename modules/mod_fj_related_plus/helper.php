<?php
/**
 * @version		$Id: helper.php 40 2010-05-30 21:42:44Z dextercowley $
...
class modFJRelatedPlusHelper
{
	...
	function getList($params)
	{
		...
		if (modFJRelatedPlusHelper::isArticle() || ($includeMenuTypes == 'any')) //only show for article pages
		{
			...
			// process sections and categories either as comma-delimited list or as array
			// (for backward compatibility)
			$catid = (is_array($params->get('catid'))) ?
				implode(',', $params->get('catid') ) : trim($params->get('catid'));
			$secid = (is_array($params->get('secid'))) ?
				implode(',', $params->get('secid') ) : trim($params->get('secid'));
			...
			$includeSections	= (is_array($params->get('fj_include_sections')))
				? implode(',', $params->get('fj_include_sections')) : $params->get('fj_include_sections');
			...
			if (modFJRelatedPlusHelper::isArticle()) {
				...
				$query = 'SELECT a.metakey, a.sectionid, a.catid, a.created_by, a.created_by_alias,' .
			...
			if (($metakey) || 	// do the query if there are keywords
				...
				($includeSections > ' ') || // or we are including other sections
				...
				($includeAliases > ' ')) // or other author aliases		
			{
				...
				($includeAliases > ' ')) // or other author aliases				
				{
					...
					switch ($orderBy)
					{
						...
						case 'article_order' :
							$sqlSort = 'ORDER BY s.ordering, cc.ordering, a.ordering, a.title' ;
							break;
					...
					if ($secid > ' ' and ($mainArticle->sectionid > ' ')) {
						$ids = str_replace('C', $mainArticle->sectionid, JString::strtoupper($secid));
						$ids = explode( ',', $ids );
						JArrayHelper::toInteger( $ids );
						$secCondition = ' AND a.sectionid IN(' . implode(',', $ids ) . ')';
					}
					...
					if ($includeSections > ' ') {
						$includeSectionsCondition = ' OR a.sectionid in ('. $includeSections . ') ';
					}
					...
					$dateSelected .
					 		' a.sectionid, a.catid, cc.access AS cat_access,' .
							...
							' cc.title as category_title, a.introtext as introtext_raw, a.fulltext, s.title as section_title,' .
							$random . 						
					...
					(($includeSections > ' ') ? $includeSectionsCondition : ''). // include articles from these sections
					(($includeCategories > ' ') ? $includeCategoriesCondition : ''). // include articles from these sections
					(($includeAuthors > ' ') ? $includeAuthorCondition : ''). // include articles from these sections
					(($includeAliases > ' ') ? $includeAliasCondition : ''). // include articles from these sections
					' )' .
							' AND ( a.publish_up = '.$db->Quote($nullDate).' OR a.publish_up <= '.$db->Quote($now).' )' .
							' AND ( a.publish_down = '.$db->Quote($nullDate).' OR a.publish_down >= '.$db->Quote($now).' ) ' . 
					((($secid > ' ') && ($mainArticle->sectionid > ' ')) ? $secCondition : '') .  // add section selection, if any
					...
					if (count($temp)) // we have at least one related article
					{
							...
							($orderBy == 'keyword_article') || ($minimumMatches > 1))
						{
							foreach ($temp as $row) // loop through each related article
							{
								...
								// add include section, category, author, and alias if not already on the list
								if (($includeSections > ' ') && (in_array($row->sectionid , explode(',', $includeSections)))) {
									$row->match_count++;
									$matching_keywords[] = ($row->sectionid == 0) ? JText::_('Uncategorised Section') : trim($row->section_title);
								}
								if (($includeCategories > ' ') 
						...
						foreach ($temp as $row)
						{
							if (($row->cat_state == 1 || $row->cat_state == '') && ($row->sec_state == 1 || $row->sec_state == '') 
								&& ($row->cat_access <= $user->get('aid', 0) || $row->cat_access == '') 
								&& ($row->sec_access <= $user->get('aid', 0) || $row->sec_access == '')
								&& ($row->match_count >= $minimumMatches || $minimumMatches <= 1))
							{
